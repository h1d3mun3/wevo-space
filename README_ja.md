...
タイムスタンプは署名対象のメッセージに含まれるため、署名後に改ざんすることはできません。ただし、値そのものは署名クライアントが主張するものであり、サーバーは信頼できる時刻ソースと照合しません。システムクロックを操作したクライアントが任意のタイムスタンプを含む有効な署名を送信しても、サーバーはこれを受け入れます。

標準的な解決策（RFC 3161 タイムスタンプ局）は外部への依存を生むため、Wevo の設計原則と相容れません。これは既知のトレードオフです。有効期限など時刻に基づく機能を将来追加する場合は、この制限を考慮する必要があります。
