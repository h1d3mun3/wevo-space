...
タイムスタンプ（`createdAt`・`timestamp` フィールド）は署名対象のメッセージに含まれるため、**署名後に改ざんすることはできません**。ただし、値そのものは署名クライアントが主張するものであり、サーバーは信頼できる時刻ソースと照合しません。

システムクロックを操作したクライアントが任意のタイムスタンプを含む有効な署名を送信した場合、サーバーはこれを受け入れます。

これは既知のトレードオフです（標準的な解決策である RFC 3161 タイムスタンプ局は外部依存を生み、Wevo の設計原則と相容れない）。有効期限など時刻に基づく機能を将来追加する場合は、この制限を必ず考慮してください。
